...
| Novo importador / layout de planilha | `src/Domain/Importacao/PlanilhaOrcamentoParser.php` | `src/Infrastructure/PlanilhaLoader.php`, `helpers/file_import.php`, `tests/run.php` |
| Prévia de obra (regras de estimativa) | `src/Domain/Estimativa/EstimativaService.php` | `Classificador.php` (etapas/materiais/tipologias), `Incc.php` (índices), `PlantaAnalyzer.php`, `admin/previa_obra.php` |
| Base de referência | `src/Domain/Estimativa/ReferenciaService.php` | `scripts/acervo.php`, `database/referencias/`, `admin/referencias.php` |
| Ciclo de vida do orçamento | `src/Domain/Orcamento/OrcamentoService.php`, `OrcamentoStatus.php` | `admin/orcamento_detalhe.php`, `admin/orcamento_editar.php`, `helpers/orcamento_editor.php` |
...
1. **Unificar validação de uploads de resposta de cotação.**
   Parcialmente feito na 1.3.0: extensão permitida, limite de tamanho, nome aleatório e entrega por `admin/cotacao_arquivo.php`. Falta extrair um serviço e validar o MIME real com `finfo`.

2. **Validar estados no servidor.**
   Orçamento concluído na 1.3.0 (`OrcamentoStatus`) e compra valida a lista de status. Falta obra, etapa e cotação.

3. **Transacionar consolidação de cotação.**
   Concluído na 1.3.0: resposta manual e leitura de cotação gravam em uma transação.

4. **Evitar duplicidade na reimportação de cotação.**
   Concluído na 1.3.0: uma nova leitura substitui as linhas anteriores da mesma cotação.
...
5. ~~Corrigir o indicador “Cotações Recebidas” em `cliente/dashboard.php`~~ — concluído na 1.3.0 (conta `respondida` e `aceita`).
