...
requireAdmin();
$db = getDB();
$obraService = new \App\Domain\Obra\ObraService($db);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($acao, ['criar', 'editar'])) {
        ...
        $data['valor_total'] = max(0, \App\Domain\Orcamento\OrcamentoCalculator::decimal($data['valor_total']));
        foreach (['data_inicio','data_prev_fim'] as $d) if (!$data[$d]) $data[$d] = null;

        if ($acao === 'criar') {
            try {
                $obraId = $obraService->criar($data);
            } catch (Throwable $exception) {
                error_log('[obra_criar] ' . $exception);
                setFlash('error', 'Não foi possível criar a obra. Verifique os dados e tente novamente.');
                redirect(APP_URL . '/admin/obras.php');
            }
            $obraService->atualizarCaracteristicas($obraId, \App\Domain\Orcamento\OrcamentoCalculator::decimal($_POST['area_construida'] ?? 0) ?: null, $_POST['tipologia'] ?? null, $_POST['padrao'] ?? null);
            logAction('obra_criada','obras',$obraId,$data['nome']);
            setFlash('success','Obra criada com sucesso.');
        } else {
            $id = (int)($_POST['obra_id'] ?? 0);
            try {
                $obraService->atualizar($id, $data);
            } catch (Throwable $exception) {
                error_log('[obra_editar] ' . $exception);
                setFlash('error', 'Não foi possível atualizar a obra. Verifique os dados e tente novamente.');
                redirect(APP_URL . '/admin/obras.php');
            }
            $obraService->atualizarCaracteristicas($id, \App\Domain\Orcamento\OrcamentoCalculator::decimal($_POST['area_construida'] ?? 0) ?: null, $_POST['tipologia'] ?? null, $_POST['padrao'] ?? null);
...
$limit   = 15;
$offset  = ($pag - 1) * $limit;
$ordem = ordenacaoAtual(['nome' => 'o.nome', 'cliente' => 'c.razao_social', 'progresso' => 'o.progresso', 'valor' => 'o.valor_total', 'previsao' => 'o.data_prev_fim', 'data' => 'o.criado_em'], 'data');

$wheres = []; $params = [];
if ($clienteFilter) { $wheres[] = 'o.cliente_id = ?'; $params[] = $clienteFilter; }
if ($search !== '') $wheres[] = \App\Domain\Consulta\TermoBusca::condicao(['o.nome', "COALESCE(o.cidade,'')", "COALESCE(o.endereco,'')", 'c.razao_social', "COALESCE(o.tipologia,'')"], \App\Domain\Consulta\TermoBusca::termos($search), $params);
$from = 'FROM obras o JOIN clientes c ON c.id=o.cliente_id';
$contagens = $db->prepare("SELECT o.status, COUNT(*) n $from " . ($wheres ? 'WHERE ' . implode(' AND ', $wheres) : '') . ' GROUP BY o.status');
...
$stmt = $db->prepare("SELECT o.*, c.razao_social $from $where ORDER BY {$ordem['sql']}, o.id DESC LIMIT $limit OFFSET $offset");
$stmt->execute($params);
$obras = $stmt->fetchAll();
...
<div class="card">
    ...
    <form method="get" class="query-bar" role="search">
        <?php foreach (['status' => $status, 'ordem' => $_GET['ordem'] ?? '', 'dir' => $_GET['dir'] ?? ''] as $k => $v): if ($v === '') continue; ?>
        <input type="hidden" name="<?= $k ?>" value="<?= sanitize((string) $v) ?>">
        <?php endforeach; ?>
        <label class="query-search">
            <i class="fa-solid fa-magnifying-glass" aria-hidden="true"></i>
            <input type="search" name="q" value="<?= sanitize($search) ?>" placeholder="Buscar por obra, cidade, endereço, cliente ou tipologia…" aria-label="Buscar obras" data-autosubmit>
        </label>
        <select name="cliente_id" class="form-control" aria-label="Cliente" data-autosubmit>
            <option value="">Todos os clientes</option>
            <?php foreach ($clientes as $cl): ?><option value="<?= $cl['id'] ?>" <?= $clienteFilter === (int) $cl['id'] ? 'selected' : '' ?>><?= sanitize($cl['razao_social']) ?></option><?php endforeach; ?>
        </select>
        <button class="btn btn-outline btn-sm">Filtrar</button>
    </form>
    <?= abasStatus($abas, $status) ?>
    <?php $clienteNome = ''; foreach ($clientes as $cl) if ((int) $cl['id'] === $clienteFilter) $clienteNome = $cl['razao_social']; ?>
    <?= chipsFiltros(['q' => $search !== '' ? 'Busca: “' . $search . '”' : '', 'cliente_id' => $clienteNome !== '' ? 'Cliente: ' . $clienteNome : '']) ?>
    <div class="table-wrap">
        <table>
            <thead><tr>
                <?= thOrdenavel('Obra', 'nome', $ordem) ?><?= thOrdenavel('Cliente', 'cliente', $ordem) ?><th>Status</th>
                <?= thOrdenavel('Progresso', 'progresso', $ordem) ?><?= thOrdenavel('Valor', 'valor', $ordem) ?><?= thOrdenavel('Previsão', 'previsao', $ordem) ?><th>Ações</th>
            </tr></thead>
            <tbody>
            <?php foreach ($obras as $o): ?>
            <tr>
                <td>
                    <a href="<?= APP_URL ?>/admin/obra_detalhe.php?id=<?= $o['id'] ?>" class="font-bold"><?= \App\Domain\Consulta\TermoBusca::destacar(trim($o['nome']), $termos) ?></a>
                    <div class="text-xs text-muted"><?= $o['cidade'] ? sanitize($o['cidade'] . ($o['estado'] ? '/' . $o['estado'] : '')) : '' ?></div>
                </td>
                <td class="text-sm"><?= \App\Domain\Consulta\TermoBusca::destacar(trim($o['razao_social']), $termos) ?></td>
                ...
                <td>
                    <div class="td-actions">
                        ...
                        <button class="btn btn-sm btn-outline" onclick='editarObra(<?= jsonAttr($o) ?>)' title="Editar"><i class="fa-solid fa-pen"></i></button>
            ...
            <?php endforeach; ?>
            <?php if (!$obras): ?><tr><td colspan="7" class="empty-cell"><i class="fa-solid fa-magnifying-glass" aria-hidden="true"></i><strong>Nenhuma obra encontrada.</strong><span><a href="<?= APP_URL ?>/admin/obras.php">Limpar filtros</a></span></td></tr><?php endif; ?>
            </tbody>
        </table>
    </div>
            ...
            <div class="modal-body">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="valor_total" id="fObraValor" class="form-control" step="0.01" min="0" placeholder="0,00">
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label class="form-label">Área construída (m²)</label>
                        <input type="number" name="area_construida" id="fObraArea" class="form-control" step="any" min="0">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Tipologia</label>
                        <select name="tipologia" id="fObraTipologia" class="form-control">
                            <option value="">--</option>
                            <?php foreach (\App\Domain\Estimativa\Classificador::TIPOLOGIAS as $chaveTip => $rotuloTip): ?><option value="<?= $chaveTip ?>"><?= sanitize($rotuloTip) ?></option><?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Padrão</label>
                        <select name="padrao" id="fObraPadrao" class="form-control">
                            <option value="">--</option><option value="economico">Econômico</option><option value="medio">Médio</option><option value="alto">Alto</option>
                        </select>
                    </div>
                </div>
                <div class="form-row">
...
function editarObra(o) {
    ...
    document.getElementById('fObraArea').value   = o.area_construida || '';
    document.getElementById('fObraTipologia').value = o.tipologia || '';
    document.getElementById('fObraPadrao').value = o.padrao || '';
    openModal('modalObra');
}
</script>
