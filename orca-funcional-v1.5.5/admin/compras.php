...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($acao,['criar','editar'])) {
        ...
        $valor   = max(0, \App\Domain\Orcamento\OrcamentoCalculator::decimal($_POST['valor_total'] ?? 0));
        if (!in_array($status, ['solicitado','confirmado','em_producao','enviado','entregue','cancelado'], true)) $status = 'solicitado';
            ...
            <tr>
                ...
                <td>
                    <div class="td-actions">
                        <button class="btn btn-sm btn-outline" onclick='editarCompra(<?= jsonAttr($cp) ?>)'>✏️</button>
