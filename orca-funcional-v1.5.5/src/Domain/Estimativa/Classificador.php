...
final class Classificador
{
    ...
    public static function tipologia(string $texto): string
    {
        ...
        return match (true) {
            (bool) preg_match('/\b([2-9]|\d{2,}|duas|tres|quatro) (casas|residencias)\b|geminad/', $t) => 'multifamiliar',
            (bool) preg_match('/residencia unifamiliar|casa unifamiliar/', $t) => 'residencial',
            (bool) preg_match('/retrofit|refrofit|\breforma\b|manutencao e recomposicao|recuperacao estrutural|manutencao predial/', $t) => 'reforma',
            (bool) preg_match('/multifamiliar|sobrado|edificio|\bed\. |predio|condominio|\d unidades|apartament|\bhis\b/', $t) => 'multifamiliar',
            (bool) preg_match('/ampliac/', $t) => 'reforma',
            ...
            (bool) preg_match('/clinica|hotel|\bupa\b|hemosc|hemocentro|hospital|posto de saude|bistro|comercial|escritorio|loja|salao|galpao|escola|institucional|guarita|zeladoria|centro cultural|igreja|biblioteca/', $t) => 'comercial',
            default => 'residencial',
        };
    }

    /**
     * Tipologia pelo texto das pranchas: conta as menções de cada uso (o texto de uma planta
     * traz notas e legendas genéricas, então uma palavra isolada não decide).
     */
    public static function tipologiaDaPlanta(string $texto, ?float $area = null): string
    {
        $t = PlanilhaOrcamentoParser::normalizar($texto);
        $conta = static fn (string $padrao): int => (int) preg_match_all($padrao, $t);
        $edificacoes = count(array_unique(preg_match_all('/\bedificacao 0(\d)\b/', $t, $m) ? $m[1] : []));
        $pontos = [
            'multifamiliar' => 3 * $conta('/multifamiliar|pavimento tipo|apart ?hotel/') + $conta('/\bapto\b|apartamento/') + ($edificacoes >= 2 ? 6 : 0) + 3 * $conta('/\b\d+ (casas|unidades)\b|geminad/'),
            'residencial' => 3 * $conta('/unifamiliar/') + $conta('/\bsuite\b|dormitorio/'),
            'comercial' => 2 * $conta('/\b(hotel|clinica|consultorio|loja|sala comercial|escritorio|recepcao|salao|galpao|escola|igreja|restaurante|bistro)\b/'),
            'reforma' => 3 * $conta('/a demolir|a construir|existente a manter|reforma\b/'),
            'esportivo' => 3 * $conta('/\b(quadra|arena|ginasio)\b/'),
            'urbanizacao' => 3 * $conta('/\b(praca|loteamento|parque|paisagismo)\b/'),
        ];
        arsort($pontos);
        $tipologia = (int) reset($pontos) > 0 ? (string) key($pontos) : 'residencial';
        if ($tipologia === 'residencial' && $area !== null && $area > 1200) {
            $tipologia = 'multifamiliar'; // residência acima de 1.200 m² costuma ser edifício/condomínio
        }
        return $tipologia;
    }
