...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['acao'] ?? '') === 'importar') {
    ...
    if (isset($_FILES['arquivo']) && $_FILES['arquivo']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!$itens) $importErro = 'Nenhum item encontrado. Verifique se o arquivo é uma planilha padrão CAIXA/SINAPI.';
    } else {
        ...
        <div class="table-wrap">
            <table>
                <thead><tr>
                    ...
                    <th>Preço Unit. (R$)</th><th>Total Est.</th><th>Categoria</th>
                </tr></thead>
                <tbody>
                ...
                <tr>
                    ...
                    <td><select name="item_cat[]" class="form-control" style="font-size:.8rem;padding:4px 6px">
                        ...
                        <?php foreach ($categorias as $cat): ?><option value="<?= $cat['id'] ?>"><?= sanitize($cat['nome']) ?></option><?php endforeach; ?>
                    </select></td>
                </tr>
                <?php endforeach; ?>
                </tbody>
                <tfoot>
                    <tr style="background:var(--neutral-50);font-weight:700">
                        ...
                        <td colspan="2">R$ <?= number_format($totGeral,2,',','.') ?></td>
                    </tr>
                </tfoot>
            </table>
        </div>
