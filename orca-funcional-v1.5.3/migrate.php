...
try {
    ...
    $uploads = $installer->migrateLegacyUploads();
    $db->exec(
    ...
    // Base de referência da prévia de obra (acervo anonimizado + SINAPI), idempotente
    $referencias = (new \App\Domain\Estimativa\ReferenciaService($db))->carregarPasta(__DIR__ . '/database/referencias');
    echo json_encode(['ok' => true, 'version' => APP_VERSION, 'migrations' => $novas, 'seeded' => $seeded, 'uploads' => $uploads, 'referencias' => $referencias], JSON_UNESCAPED_UNICODE);
