...
- a tela mostra aba usada, etapas, BDI detectado e o total informado na planilha; divergências acima de 0,5% geram aviso para conferência;
...
| Formato | Estratégia |
|---|---|
| XLSX/XLS/ODS | `PlanilhaLoader` lê todas as abas com valores calculados; `PlanilhaOrcamentoParser` localiza cabeçalho e sub-cabeçalho, etapas, subtotais, material + mão de obra, BDI e total geral e escolhe a aba mais completa. |
| CSV | Separador `;` ou `,` detectado; mesmo parser das planilhas. |
...
No detalhe do orçamento: **Editar itens** (enquanto não aprovado/cancelado), **Nova revisão** (copia itens e BDI para um rascunho ligado por `revisao_de`), **Aprovar/Reprovar/Cancelar/Reabrir como rascunho** (transições validadas) e **Excluir** (bloqueado quando há compras ligadas às cotações).

## 4.1 Prévia de obra

```mermaid
flowchart LR
    P["Planta PDF"] --> A["PlantaAnalyzer: área, esquadrias, ambientes"]
    Q["Planilha de quantitativos"] --> R["Parser + precificação por semelhança"]
    A --> E["EstimativaService: comparáveis, R$/m², etapas, materiais"]
    B[("Base: referências reais + SINAPI")] --> E
    B --> R
    E --> V["Tela de prévia"]
    R --> V
    V --> O["Orçamento em rascunho + planta salva na obra"]
```

1. `admin/previa_obra.php` recebe planta (até 80 MB) e/ou planilha, área, tipologia e padrão; os arquivos ficam em `uploads/previas/<token>` por até 2 dias e o token é validado na sessão.
2. Área: quadro de áreas da planta; na falta, soma dos ambientes +12% ou valor informado.
3. Comparáveis: referências ativas com área da mesma tipologia (ou do mesmo grupo), atualizadas pelo INCC e ponderadas por porte e data; padrão econômico/médio/alto = percentis 25/50/75.
4. Etapas: participação mediana por etapa padronizada; materiais: consumo mediano por m² e preço mediano; esquadrias da planta: preço por unidade (portas) ou m² (janelas).
5. Urbanização e infraestrutura não recebem custo por m²; a tela pede a planilha de quantitativos, precificada item a item.
6. "Gerar orçamento" cria um rascunho com materiais, esquadrias e uma linha de "demais serviços" por etapa, fechando com o total da prévia.
7. Orçamentos aprovados podem ser incluídos na base ("Usar como referência").
...
4. A cotação vira `respondida`; o orçamento vira `cotado` se ainda estava em rascunho/aguardando. Tudo ocorre em uma transação; o arquivo de resposta é validado, recebe nome aleatório e é aberto por `admin/cotacao_arquivo.php`.
...
Uma nova leitura da mesma cotação substitui as linhas anteriores em `cotacao_itens`. Quando várias respostas atualizam o mesmo item, a última gravação vence. Não existe ainda uma regra automática de “menor preço com condições equivalentes”.
