...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['acao'] ?? '') === 'importar') {
    ...
    if (isset($_FILES['arquivo']) && $_FILES['arquivo']['error'] === UPLOAD_ERR_OK) {
        $importacaoCaixa = importarPlanilhaDetalhada($_FILES['arquivo']['tmp_name'], strtolower(pathinfo((string) $_FILES['arquivo']['name'], PATHINFO_EXTENSION)));
        $itens = $importacaoCaixa['itens'];
        $bdiDetectado = (float) $importacaoCaixa['bdi_percentual'];
        if (!$itens) {
            $importErro = $importacaoCaixa['erro'] ?? 'Nenhum item encontrado. Verifique se o arquivo é uma planilha padrão CAIXA/SINAPI.';
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['acao'] ?? '') === 'criar_orcamento') {
    ...
    $itensParaSalvar = orcamentoItensDoPost($_POST);
    ...
    try {
        ...
        $orcId = $service->criar($obraId, $titulo, 'caixa', '', $itensParaSalvar, $bdi);
...
<div class="card">
    ...
    <form method="post">
        ...
        <input type="hidden" name="bdi_percentual" value="<?= $bdiDetectado ?>">
        ...
        <div class="table-wrap">
            <table>
                ...
                <tr>
                    <td class="text-xs text-muted"><input type="hidden" name="item_codigo[]" value="<?= sanitize($item['codigo'] ?? '') ?>"><input type="hidden" name="item_etapa[]" value="<?= sanitize($item['etapa'] ?? '') ?>"><?= sanitize($item['codigo'] ?? '') ?><?php if (!empty($item['etapa'])): ?><div class="text-xs"><?= sanitize($item['etapa']) ?></div><?php endif; ?></td>
                    ...
                    <td><input type="number" name="item_preco[]" value="<?= $item['preco_unitario'] ?>" step="any" class="form-control preco-input" style="width:110px;font-size:.85rem;padding:5px 7px"></td>
