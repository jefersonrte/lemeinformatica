...
Esta pasta é a fonte oficial de conhecimento da versão `1.5.0`. Ela descreve o sistema como ele está implementado no código, sem incluir credenciais, chaves, dados pessoais ou cópias do banco de produção.
...
- orçamento manual ou importado de Excel, CSV, XML, PDF e planilha CAIXA/SINAPI, com etapas, BDI e revisões;
- prévia de obra: planta (PDF) e/ou quantitativos → materiais, quantidades, custo por etapa e orçamento em rascunho, com base de orçamentos reais anonimizados e SINAPI;
- ciclo do orçamento: rascunho, cotação, aprovação/reprovação, cancelamento, reabertura, edição e nova revisão;
