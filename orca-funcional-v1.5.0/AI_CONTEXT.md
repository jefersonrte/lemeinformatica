...
- versão funcional publicada no momento desta documentação: `/orca-funcional-v1.5.0/`.
...
- cálculos/regras extraídas: `src/Domain/` (importação de planilhas em `src/Domain/Importacao/PlanilhaOrcamentoParser.php`, estados em `src/Domain/Orcamento/OrcamentoStatus.php`);
...
- Planilhas e plantas de clientes nunca entram no Git; apenas a base anonimizada em `database/referencias/` gerada por `scripts/acervo.php referencias` (sem nomes, endereços ou contatos).
