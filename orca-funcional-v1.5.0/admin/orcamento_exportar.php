...
$statement = $db->prepare(
    'SELECT o.titulo, o.total_estimado, o.bdi_percentual, ob.nome AS obra, c.razao_social FROM orcamentos o '
...
$itemsStatement = $db->prepare(
    ...
    . 'oi.preco_total, oi.preco_cotado, oi.total_cotado, oi.etapa, cat.nome AS categoria '
    . 'FROM orcamento_itens oi LEFT JOIN categorias cat ON cat.id = oi.categoria_id '
    . 'WHERE oi.orcamento_id = ? ORDER BY oi.ordem, oi.id'
);
...
fputcsv($output, [], ';', '"', '');
fputcsv($output, ['Etapa', 'Código', 'Descrição', 'Categoria', 'Unidade', 'Quantidade', 'Preço unitário', 'Total estimado', 'Preço cotado', 'Total cotado'], ';', '"', '');
foreach ($itemsStatement->fetchAll() as $item) {
    fputcsv($output, [
        $item['etapa'], $item['codigo'], $item['descricao'], $item['categoria'], $item['unidade'],
...
$bdi = (float) ($orcamento['bdi_percentual'] ?? 0);
$custoDireto = (float) $orcamento['total_estimado'];
fputcsv($output, [], ';', '"', '');
fputcsv($output, ['', '', 'Custo direto', '', '', '', '', number_format($custoDireto, 2, ',', '')], ';', '"', '');
if ($bdi > 0) {
    fputcsv($output, ['', '', 'BDI (' . number_format($bdi, 2, ',', '') . '%)', '', '', '', '', number_format(\App\Domain\Orcamento\OrcamentoService::totalComBdi($custoDireto, $bdi) - $custoDireto, 2, ',', '')], ';', '"', '');
    fputcsv($output, ['', '', 'Total com BDI', '', '', '', '', number_format(\App\Domain\Orcamento\OrcamentoService::totalComBdi($custoDireto, $bdi), 2, ',', '')], ';', '"', '');
}
fclose($output);
exit;
