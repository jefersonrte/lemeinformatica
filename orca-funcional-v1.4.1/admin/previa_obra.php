...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($acao === 'analisar') {
        ...
        foreach (['planta' => ['pdf', 'png', 'jpg', 'jpeg', 'webp'], 'planilha' => ['xlsx', 'xls', 'xlsm', 'ods', 'csv']] as $campo => $extensoes) {
            ...
            if ($arquivo['error'] !== UPLOAD_ERR_OK || !is_uploaded_file($arquivo['tmp_name']) || $arquivo['size'] > $limite || !in_array($ext, $extensoes, true)) {
                setFlash('error', ($campo === 'planta' ? 'Planta' : 'Planilha') . ' inválida: use ' . implode(', ', $extensoes) . ' até ' . $limiteMb . 'MB.');
                redirect(APP_URL . '/admin/previa_obra.php');
            }
            $alvo = $destino . '/' . $campo . '.' . $ext;
            move_uploaded_file($arquivo['tmp_name'], $alvo);
            $entrada[$campo] = $alvo;
            $entrada[$campo === 'planta' ? 'planta_nome' : 'planilha_ext'] = $campo === 'planta' ? (string) $arquivo['name'] : ($ext === 'xlsm' ? 'xlsx' : $ext);
            file_put_contents($destino . '/' . $campo . '.nome', mb_substr(basename((string) $arquivo['name']), 0, 200));
        }
        if (empty($entrada['planta']) && empty($entrada['planilha']) && empty($entrada['area'])) {
        ...
        try {
            ...
            if (!empty($_POST['salvar_planta']) && $pasta) {
                foreach (glob($pasta . '/planta.*') ?: [] as $arquivoPlanta) {
                    if (str_ends_with($arquivoPlanta, '.nome')) {
                        continue;
                    }
                    $nome = is_file($pasta . '/planta.nome') ? (string) file_get_contents($pasta . '/planta.nome') : basename($arquivoPlanta);
                    try {
    ...
    <form method="post" enctype="multipart/form-data" class="card-body" id="formPrevia">
        ...
        <div class="form-row">
            <div class="form-group">
                <label class="form-label">Planta arquitetônica (PDF)</label>
                <input type="file" name="planta" class="form-control" accept=".pdf,.png,.jpg,.jpeg,.webp">
                <p class="form-text">Lê a área construída do quadro de áreas, os ambientes e o quadro de esquadrias (até <?= max(UPLOAD_MAX_MB, 80) ?> MB).</p>
            </div>
            <div class="form-group">
