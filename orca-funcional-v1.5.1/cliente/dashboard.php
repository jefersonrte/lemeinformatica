...
$cotacoesResp   = $db->prepare("SELECT COUNT(*) FROM cotacoes co JOIN orcamentos o ON o.id=co.orcamento_id WHERE o.cliente_id=? AND co.status IN ('respondida','aceita')"); $cotacoesResp->execute([$cliente['id']]); $cotacoesResp = $cotacoesResp->fetchColumn();
