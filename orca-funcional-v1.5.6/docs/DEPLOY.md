...
| Produção atual | `/orca-funcional-v1.5.5/` | Versão funcional recomendada. |
| Histórico | `/orca-funcional-v1.5.4/`, `/orca-funcional-v1.5.3/`, `/orca-funcional-v1.5.2/`, `/orca-funcional-v1.5.1/`, `/orca-funcional-v1.5.0/`, `/orca-funcional-v1.4.1/`, `/orca-funcional-v1.4.0/`, `/orca-funcional-v1.3.0/`, `/orca-funcional-v1.2.4/`, `/orca-funcional-v1.2.3/`, `/orca-funcional-v1.2.2/`, `/orca-funcional-v1.2.1/`, `/orca-funcional-v1.2.0/`, `/orca-v1.2.0/`, `/orca/` | Versões anteriores preservadas para consulta/rollback. |
...
## 7.1 Base de referência da prévia

`migrate.php` também carrega os arquivos de `database/referencias/*.json` (acervo anonimizado e SINAPI) com INSERTs em lote; a carga é idempotente (`fonte_hash` e chave única de preços). Para atualizar a base a partir de novas planilhas, na máquina local:

```bash
php scripts/acervo.php inventario ../planilhas
php scripts/acervo.php referencias ../planilhas database/referencias/acervo-orcamentos-reais.json
php scripts/acervo.php sinapi "caminho/Referência MM-AAAA.xlsm" database/referencias/sinapi-<local>-<AAAA-MM>.json
```

Revise o JSON (sem nomes de clientes, endereços ou contatos) antes do commit. `php scripts/acervo.php importar ../planilhas` cria clientes, obras, plantas e orçamentos nomeados somente no banco configurado localmente.
