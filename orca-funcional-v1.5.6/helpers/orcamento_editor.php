...
function validarItens() {
    ...
    // Os itens seguem num único campo JSON: o servidor limita a quantidade de campos por formulário
    // (max_input_vars) e orçamentos com centenas de linhas seriam cortados.
    const valor = (r, nome) => (r.querySelector(`[name="${nome}[]"]`) || {}).value ?? '';
    const itens = Array.from(linhas).map(r => ({
        id: valor(r, 'item_id'), etapa: valor(r, 'item_etapa'), codigo: valor(r, 'item_codigo'), descricao: valor(r, 'item_desc'),
        unidade: valor(r, 'item_un'), quantidade: valor(r, 'item_qtd'), preco_unitario: valor(r, 'item_preco'), categoria_id: valor(r, 'item_cat'),
    }));
    let campo = document.getElementById('itensJson');
    if (!campo) {
        campo = document.createElement('input');
        campo.type = 'hidden';
        campo.name = 'itens_json';
        campo.id = 'itensJson';
        campo.setAttribute('form', EDITOR_FORM);
        document.getElementById('tabelaItens').before(campo);
    }
    campo.value = JSON.stringify(itens);
    document.querySelectorAll('#itensBody [name^="item_"]').forEach(el => { el.disabled = true; });
    window.addEventListener('pageshow', () => document.querySelectorAll('#itensBody [name^="item_"]').forEach(el => { el.disabled = false; }), { once: true });
    return true;
}
</script>
    <?php
...
{
    // Formato atual do editor: todos os itens num campo JSON (não esbarra no max_input_vars)
    if (isset($post['itens_json']) && is_string($post['itens_json']) && $post['itens_json'] !== '') {
        $lista = json_decode($post['itens_json'], true);
        if (!is_array($lista)) {
            throw new InvalidArgumentException('Não foi possível ler os itens do orçamento.');
        }
        $itens = [];
        foreach (array_values($lista) as $i => $item) {
            ...
            $itens[] = [
                ...
                'categoria_id' => ($item['categoria_id'] ?? '') !== '' ? $item['categoria_id'] : null,
