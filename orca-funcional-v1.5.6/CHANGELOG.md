...
### Acervo cadastrado pelas telas
- **Importar acervo → "Cadastrar um por um pelas telas"**: para cada obra do pacote, cadastra o cliente pela tela de Clientes, a obra pela tela de Obras e cada orçamento pela tela de Novo orçamento (com status aprovado/cancelado pela tela do orçamento), confere total e quantidade de itens gravados e só então anexa as plantas;
- o importador reconhece obras (cliente + nome) e orçamentos (obra + título) já digitados pelas telas: nada é duplicado ao anexar as plantas ou reenviar o pacote.

### Clientes
- cliente pode ser cadastrado **sem acesso ao portal** (senha em branco): usuário fica inativo; definir uma senha em Editar libera o acesso.

### Orçamentos
- o editor de itens envia as linhas num único campo JSON: orçamentos com mais de ~200 itens eram cortados ao salvar pelo limite de campos do PHP (`max_input_vars`); o formato antigo continua aceito.

## 1.5.5 — 2026-09-24

### Busca global (Ctrl+K)
- campo de busca na barra superior e atalho **Ctrl+K** (⌘K no Mac): paleta com resultados agrupados enquanto se digita e buscas recentes;
- procura em orçamentos, obras e itens de orçamento; o administrador também vê clientes, fornecedores e produtos;
- todas as palavras precisam aparecer, em qualquer ordem, com ou sem acento; os trechos encontrados ficam destacados;
- página **Busca** com a lista completa por grupo; o cliente só enxerga as próprias obras e orçamentos.

### Consulta de preços
- nova tela **Consulta de preços** (menu Orçamentos e área do cliente): histórico dos preços praticados nos itens dos orçamentos;
- mediana, faixa usual (quartis), mínimo e máximo por unidade; comparação de cada preço com a mediana;
- filtros por unidade, etapa e status, ordenação por relevância, preço ou data e **Exportar CSV**.

### Cotação
- **Fornecedor rápido**: cadastro de fornecedor sem sair do painel de cotação do orçamento;
- cotação dos itens escolhidos; na resposta aparecem só os itens pedidos, com opção de ver todos;
- link de WhatsApp com DDI do Brasil validado.

### Interface
- paginação com links e reticências (1 … 4 5 [6] 7 8 … 20).

## 1.5.4 — 2026-09-24

### Junção das plantas
- botão **Unir com IA** em Plantas (com uma obra filtrada) abre a nova tela **Junção das plantas**;
- animação em SVG com as cores do tema ativo: as pranchas chegam, se espalham pela mesa, são lidas (faixa de leitura e selo com o dado encontrado), ligadas às vizinhas e encaixadas numa imagem única com o nome de cada prancha;
- PDFs mostrados pela 1ª página (pdf.js, no navegador); imagens no formato original — o formato das peças segue a mediana das pranchas (em pé ou deitadas);
- ao final: área construída, pavimentos, ambientes e esquadrias unidos de todas as pranchas em PDF (`PlantaAnalyzer::fundir`), com aviso quando as pranchas são imagens sem texto;
- **Baixar imagem unida** gera um PNG em alta resolução com todas as pranchas lado a lado;
- a leitura de cada PDF fica guardada ao lado do arquivo (`*.analise.json`) e é refeita quando o PDF ou as regras de leitura mudam; as leituras vão uma por vez para não atrasar o download das pranchas.

## 1.5.3 — 2026-09-24

### Importar acervo de obras
- nova tela **Importar acervo** (Base de referência → "Importar acervo de obras"): recebe o pacote gerado por `php scripts/acervo.php pacote acervo.zip` e cria no sistema os clientes fictícios, as obras, os orçamentos reais (itens, quantidades, preços, BDI e status) e as plantas de cada obra;
- envio em partes de 4 MB (pacotes de centenas de MB passam pelo limite de upload do servidor) e importação obra por obra com progresso na tela;
- usa os mesmos serviços das telas (obra com as etapas padrão, orçamento com totais recalculados, planta validada e versionada no catálogo técnico);
- idempotente: orçamentos reconhecidos pelo arquivo de origem e plantas por nome e tamanho — reenviar o pacote só completa o que faltou;
- `scripts/acervo.php`: comando `pacote` e escolha das plantas de cada obra priorizando PDFs de arquitetura, sem cópias repetidas e com pranchas de até 80 MB (acervo atual: 59 obras, 120 orçamentos e 522 plantas);
- a migração de uploads entre versões ignora as pastas temporárias `acervo/` e `previas/`.
