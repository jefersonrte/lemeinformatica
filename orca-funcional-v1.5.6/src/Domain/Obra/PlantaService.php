...
final class PlantaService
{
    ...
    /**
     * @param bool $arquivoLocal true somente para arquivos gerados/copiados pelo próprio servidor
     *                           (importação do acervo e prévia de obra), nunca para dados do formulário.
     */
    public function armazenar(int $obraId, string $titulo, string $descricao, array $file, ?int $usuarioId, bool $arquivoLocal = false): int
    {
        $titulo = trim($titulo);
        if ($obraId <= 0 || $titulo === '') {
            throw new InvalidArgumentException('Obra e título da planta são obrigatórios.');
        }
        $origemValida = $arquivoLocal ? is_file((string) ($file['tmp_name'] ?? '')) : is_uploaded_file((string) ($file['tmp_name'] ?? ''));
        if (($file['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK || !$origemValida) {
        ...
        $stored = $sanitizedSvg !== null
            ...
            : ($arquivoLocal ? copy($temporaryFile, $destination) : move_uploaded_file($temporaryFile, $destination));
        if (!$stored) {
