...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($acao, ['criar', 'editar'])) {
        ...
        if (!$nome || !$email) {
            ...
            if ($acao === 'criar') {
                if ($senha !== '' && strlen($senha) < 8) { setFlash('error', 'A senha precisa de pelo menos 8 caracteres.'); redirect(APP_URL . '/admin/clientes.php'); }
                // Sem senha: cliente cadastrado sem acesso ao portal (usuário inativo com senha aleatória descartada)
                $comAcesso = $senha !== '';
                $db->beginTransaction();
                // Cria usuário
                $hash = password_hash($comAcesso ? $senha : bin2hex(random_bytes(24)), PASSWORD_BCRYPT, ['cost' => BCRYPT_COST]);
                $db->prepare('INSERT INTO usuarios (nome,email,senha,role,ativo,email_verificado) VALUES (?,?,?,?,?,1)')
                   ->execute([$nome, $email, $hash, 'cliente', $comAcesso ? 1 : 0]);
                $uid = $db->lastInsertId();
                // Cria cliente
                ...
                $clienteNovoId = $db->lastInsertId();
                $db->commit();
                logAction('cliente_criado', 'clientes', $clienteNovoId, $nome);
                setFlash('success', $comAcesso ? 'Cliente criado com sucesso.' : 'Cliente criado sem acesso ao portal (defina uma senha depois, em Editar, se ele precisar entrar).');
            } else {
                ...
                if ($senha) {
                    ...
                    $db->prepare('UPDATE usuarios SET senha=?, ativo=1 WHERE id=?')->execute([$hash, $uid_edit]); // definir senha libera o portal
                }
        ...
        error_log('[clientes] ' . $exception->getMessage());
        setFlash('error', mensagemErroBanco($exception));
    }
            ...
            <tr>
                ...
                <td>
                    <div class="td-actions">
                        ...
                        <button class="btn btn-sm btn-outline" onclick='editarCliente(<?= jsonAttr($c) ?>)' title="Editar">✏️</button>
        ...
        <form method="post" id="formCliente">
            <div class="modal-body">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <label class="form-label" id="senhaLabel">Senha do portal</label>
                        <input type="password" name="senha" id="fSenha" class="form-control" maxlength="128" autocomplete="new-password">
                        <p class="form-text" id="senhaHint">Mínimo 8 caracteres. Deixe em branco para cadastrar o cliente sem acesso ao portal.</p>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
