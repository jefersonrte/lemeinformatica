...
// Quando o pedido registrou itens específicos, mostra só esses (com opção de ver todos).
$pedidos = $db->prepare('SELECT DISTINCT orcamento_item_id FROM cotacao_itens WHERE cotacao_id=? AND orcamento_item_id IS NOT NULL');
$pedidos->execute([$id]);
$pedidos = array_map('intval', $pedidos->fetchAll(PDO::FETCH_COLUMN));
$totalItensOrcamento = count($itens);
$verTodos = isset($_GET['todos']) || !$pedidos;
if (!$verTodos) {
    $itens = array_values(array_filter($itens, static fn (array $i): bool => in_array((int) $i['id'], $pedidos, true)));
}
    ...
    <div class="card">
        ...
        <form method="post" enctype="multipart/form-data">
            <div class="card-body">
                ...
                <div class="flex justify-between items-center" style="flex-wrap:wrap;gap:8px;margin:20px 0 12px">
                    <h3 style="font-size:.9rem;font-weight:700;margin:0">Atualizar Preços Cotados:</h3>
                    <?php if ($pedidos): ?>
                    <span class="text-xs text-muted">
                        <?= $verTodos ? 'Mostrando todos os ' . $totalItensOrcamento . ' itens do orçamento.' : 'Mostrando os ' . count($itens) . ' itens pedidos a este fornecedor.' ?>
                        <a href="<?= APP_URL ?>/admin/cotacao_detalhe.php?id=<?= $id ?><?= $verTodos ? '' : '&amp;todos=1' ?>#tabResp"><?= $verTodos ? 'Ver só os pedidos' : 'Ver todos' ?></a>
                    </span>
                    <?php endif; ?>
                </div>
...
document.addEventListener('DOMContentLoaded', () => { if (location.hash === '#tabResp') switchTab('tabsCot', 'tabResp'); });
</script>
