...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($acao,['criar','editar'])) {
        ...
        $fid = (int)$_POST['fornecedor_id'];
        $vinculos = $db->prepare('SELECT (SELECT COUNT(*) FROM compras WHERE fornecedor_id=?) + (SELECT COUNT(*) FROM cotacoes WHERE fornecedor_id=?)');
        $vinculos->execute([$fid, $fid]);
        if ((int) $vinculos->fetchColumn() > 0) {
            // Excluir apagaria cotações e compras em cascata; apenas desativa.
            $db->prepare('UPDATE fornecedores SET ativo=0 WHERE id=?')->execute([$fid]);
            logAction('fornecedor_desativado','fornecedores',$fid);
            setFlash('warning','Fornecedor possui cotações ou compras e foi apenas desativado para preservar o histórico.');
        } else {
            $db->prepare('DELETE FROM fornecedores WHERE id=?')->execute([$fid]);
            logAction('fornecedor_excluido','fornecedores',$fid);
...
$search = trim($_GET['q'] ?? '');
$where  = $search ? 'WHERE (f.nome LIKE ? OR f.email LIKE ?)' : '';
$params = $search ? ["%$search%","%$search%"] : [];

$st = $db->prepare("SELECT COUNT(*) FROM fornecedores f $where");
            ...
            <tr>
                ...
                <td>
                    <div class="td-actions">
                        <button class="btn btn-sm btn-outline" onclick='editarForn(<?= $f['id'] ?>, <?= jsonAttr($f) ?>, <?= jsonAttr($vMap[$f['id']] ?? []) ?>)'>✏️</button>
