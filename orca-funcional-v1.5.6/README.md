...
- `v1.5.5`: busca global Ctrl+K, consulta de preços e fornecedor rápido na cotação.
- `v1.5.1`: tema Grafite escuro como padrão.
- `v1.5.0`: interface renovada com 6 temas (Grafite padrão), tema personalizado por degradê e 5 disposições de menu com submenus.
- `v1.4.1`: base com 42 orçamentos reais e leitura de área aprimorada.
- `v1.4.0`: prévia de obra a partir de planta e planilha (materiais, quantidades e custo aproximado) com base de 40 orçamentos reais e SINAPI.
- `v1.3.0`: importador de planilhas orçamentárias reais (etapas, BDI, material + mão de obra), gestão completa do orçamento (status, edição, revisões, exclusão) e correção dos botões e fluxos testados.
