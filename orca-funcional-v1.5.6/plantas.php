...
$db = getDB();
$admin = isAdmin();
$obraId = max(0, (int) ($_SERVER['REQUEST_METHOD'] === 'POST' ? ($_POST['obra_id'] ?? 0) : ($_GET['obra_id'] ?? 0)));
...
<section class="document-section">
    <div class="document-section-header">
        ...
        <?php if ($obraSelecionada && $plantas): ?>
        <a class="btn btn-primary btn-sm" href="<?= APP_URL ?>/plantas-ia.php?obra_id=<?= (int) $obraSelecionada['id'] ?>" title="Juntar as pranchas desta obra numa imagem única">
            <i class="fa-solid fa-wand-magic-sparkles"></i> Unir com IA
        </a>
        <?php endif; ?>
    </div>
    ...
    <div class="document-gallery">
        <?php foreach ($plantas as $plantaIndex => $planta):
            ...
            $arquivoUrl = APP_URL . '/planta-arquivo.php?id=' . (int) $planta['id']
                . '&v=' . (int) $planta['versao'] . '-' . (int) $planta['tamanho'];
        ...
        <article class="document-card" style="--card-index:<?= (int) $plantaIndex ?>">
            <button class="document-preview plant-preview-trigger" type="button"
                    data-source="<?= sanitize($arquivoUrl) ?>"
                ...
                <?php if ($imagem): ?>
                    <img src="<?= sanitize($arquivoUrl . '&thumb=1') ?>" loading="lazy" alt="Miniatura de <?= sanitize($planta['titulo']) ?>">
                <?php else: ?>
            ...
            <div class="document-body">
                ...
                <div class="document-details">
                    ...
                    <span class="document-actions">
                        ...
                        <a href="<?= sanitize($arquivoUrl) ?>" target="_blank" rel="noopener" title="Abrir documento"><i class="fa-solid fa-arrow-up-right-from-square"></i></a>
                    </span>
                </div>
            </div>
        </article>
