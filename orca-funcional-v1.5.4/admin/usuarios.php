    ...
    try {
        if (in_array($acao,['criar','editar'])) {
            ...
            if ($acao === 'criar') {
                ...
                if ($role === 'cliente') {
                    // Usuário cliente precisa de um perfil em clientes para acessar o portal
                    $db->prepare('INSERT INTO clientes (usuario_id,razao_social,email) VALUES (?,?,?)')->execute([$db->lastInsertId(),$nome,$email]);
                }
                $db->commit();
                setFlash('success', $role === 'cliente' ? 'Usuário criado. Complete os dados em Clientes.' : 'Usuário criado.');
            } else {
                $uid = (int)$_POST['usuario_id'];
                $atual = $db->prepare('SELECT role, ativo FROM usuarios WHERE id=?'); $atual->execute([$uid]); $atual = $atual->fetch();
                $deixaDeSerAdmin = $atual && $atual['role'] === 'admin' && ($role !== 'admin' || $ativo !== 1);
                if ($deixaDeSerAdmin && currentUserId() === $uid) { setFlash('error','Você não pode remover o próprio acesso de administrador.'); redirect(APP_URL.'/admin/usuarios.php'); }
                $admins = (int) $db->query("SELECT COUNT(*) FROM usuarios WHERE role='admin' AND ativo=1")->fetchColumn();
                if ($deixaDeSerAdmin && $admins <= 1) { setFlash('error','O sistema precisa manter ao menos um administrador ativo.'); redirect(APP_URL.'/admin/usuarios.php'); }
                $semPerfil = $db->prepare('SELECT COUNT(*) FROM clientes WHERE usuario_id=?'); $semPerfil->execute([$uid]);
                $db->prepare('UPDATE usuarios SET nome=?,email=?,role=?,ativo=? WHERE id=?')->execute([$nome,$email,$role,$ativo,$uid]);
                if ($role === 'cliente' && (int) $semPerfil->fetchColumn() === 0) {
                    $db->prepare('INSERT INTO clientes (usuario_id,razao_social,email) VALUES (?,?,?)')->execute([$uid,$nome,$email]);
                }
        ...
        error_log('[admin/usuarios.php] ' . $exception->getMessage());
            ...
            <tr>
                ...
                <td>
                    <div class="td-actions">
                        <button class="btn btn-sm btn-outline" onclick='editarUser(<?= jsonAttr($u) ?>)'>✏️</button>
