...
final class PlantaAnalyzer
{
    ...
    public function analisar(string $texto): array
    {
        ...
        if ($area === null && $ambientes['area_soma'] > 20) {
            ...
            $origem = 'soma das áreas dos ambientes (+12% de paredes)';
            $avisos[] = 'Área construída não informada no quadro de áreas; estimada pela soma dos ambientes.';
        }
    ...
    {
        preg_match_all('/\ba\s*[:=]\s*([\d.]+,\d{1,2})\s*m[²2]/', $t, $m);
        ...
        return [
            'total' => count($areas),
            'area_soma' => round(array_sum($areas), 2),
