...
/** Mensagem amigável para violações de integridade (duplicidade ou vínculos). */
function mensagemErroBanco(PDOException $exception): string {
    return match ((int) ($exception->errorInfo[1] ?? 0)) {
        1062 => 'Já existe um cadastro com esse e-mail ou nome.',
        1451 => 'Este registro possui itens vinculados e não pode ser excluído.',
        default => 'Não foi possível salvar. Verifique os dados e tente novamente.',
    };
}

/** JSON seguro para atributos HTML delimitados por aspas simples (ex.: onclick='fn(...)'). */
function jsonAttr(mixed $val): string {
    return (string) json_encode($val, JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_TAG | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE);
}
